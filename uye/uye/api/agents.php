...
function runYearlyChargeAgent($data)
{
    ...
    // Re-use batch_debt logic via internal call
    $_POST_BACKUP = $_POST;
    $_SERVER['REQUEST_METHOD_BACKUP'] = $_SERVER['REQUEST_METHOD'];

    // Forward to batch_debt
    $ch = curl_init();
    $baseUrl = (isset($_SERVER['HTTPS']) ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'];
    $apiUrl = $baseUrl . dirname($_SERVER['REQUEST_URI']) . '/batch_debt.php';

    curl_setopt($ch, CURLOPT_URL, $apiUrl);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_POST, true);
    curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(['year' => $year]));
    ...
    // Log the run
    global $conn;
    $logSql = "INSERT INTO agent_logs (id, agent_name, action, channel, details, status) 
               VALUES (?, 'yearly_charge', 'batch_charge', 'system', ?, ?)";
    $logStmt = $conn->prepare($logSql);
    $logId = uniqid('alog_');
    $details = "Yıl: $year - " . ($result['message'] ?? 'Sonuç bilinmiyor');
    $status = ($result['success'] ?? false) ? 'success' : 'error';
    $logStmt->bind_param('sss', $logId, $details, $status);
    $logStmt->execute();
    $logStmt->close();

    sendResponse($result['success'] ?? false, $result['message'] ?? 'Borçlandırma sonucu bilinmiyor', $result['data'] ?? null);
}
